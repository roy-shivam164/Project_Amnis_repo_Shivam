...
export class QueryBuilderCustomComponent implements OnInit{
  constructor(public dbService: DbService,public notification: ToastrService,public dialog: MatDialog){}
  ...
  */ 
  ...
  getSchemas(){
    this.reset();
    this.dbService.getSchema().subscribe(data => {
  ...
  getColumns(){
    this.selectedColumns = [];
    this.config.fields = {};
    this.dbService.getTableColumns({
    ...
    async () => {
      ...
      condition: 'and',
      rules: [
        {field: this.config.fields[Object.keys(this.config.fields)[0]].name, operator: '=', value: ""}
      ]
    }
...
saveQuery(){
  ...
  this.dbService.runStoredProcedure({
    ...
    data:{queryName:this.queryName,query:this.resultantQuery.toString()}
  })
  .subscribe(
      res => {
        console.log(res)
